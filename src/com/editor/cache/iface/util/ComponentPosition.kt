...
object ComponentPosition {
	 ...
	 */
	private fun realPosition(component: ComponentDefinition, parentWidth: Int, parentHeight: Int) {
		...
		when (component.aspectXType.toInt()) {
			...
			4 -> component.positionX = ((parentWidth - component.width) / 2 + (parentWidth * (component.basePositionX) shr 14))
			else -> component.positionX = (parentWidth - component.width - (component.basePositionX * parentWidth shr 14))
		}
		...
		when (component.aspectYType.toInt()) {
			...
			4 -> component.positionY = ((component.basePositionY * parentHeight shr 14) + (parentHeight - component.height) / 2)
			else -> component.positionY = (parentHeight - component.height - (component.basePositionY * parentHeight shr 14))
		}
		...
		if ((component.positionY + component.height) > parentHeight) component.positionY = (parentHeight - component.height)
	}
	 ...
	 */
	private fun realAfmeting(component: ComponentDefinition, width: Int, height: Int) {
	...
	fun setValues(component: ComponentDefinition?) {
		...
		val (width, height) = if (parent == null) {
